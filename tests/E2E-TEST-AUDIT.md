...
- `playerQuality` treats the player's own fallback as a manual change: when the requested level is not offered (hd2160 on the fixture, "Selected format undefined"), the player settles on another format right after the feature's request, `markManualOverride` fires ("Manual quality change detected"), enforcement is suspended and the disable path has no `currentQuality` to restore; seen as a 1-in-3 flake of the restore-after-disable case on 2026-09-04.
- `timestampPeek`: opening the preview from a comment timestamp leaves `window.scrollY` at 0 on a page that had been scrolled to the comments (2026-09-04, every candidate video), which is what the seek-on-click case asserts against before it clicks; whether the preview itself or YouTube's reaction to the seek scrolls the page is not yet established.
