...
- 2026-09-06, evening: the suite's code coverage was measured (`E2E-COVERAGE.md`): a coverage build with inline source maps, V8 coverage recorded per test with the counters flushed before every full page load, and a report mapped to the TypeScript sources. Over 717 cases the page-side code reads 83 % of statements, 65 % of branches and 90 % of functions; the weakest features are automaticallyDisableAmbientMode, playlistManagementButtons, pauseBackgroundPlayers, playerQuality and miniPlayer, the options-page input controls are barely exercised, and the background service worker is not measured at all. The harness additions (build switch, fixture, `scripts/coverage-report.mjs`) are opt-in and leave the normal build and run untouched.
- 2026-09-07: the coverage findings were worked through (`E2E-COVERAGE.md` has the re-measured numbers and what is left). The first report had under-counted: its script pre-merged the V8 records of one script across documents by adding range counts, and a range V8 leaves out means "as the enclosing block", not zero, so any block executed in one document and not another read as unexecuted; the reporter's own merge is sound and is used now, which cleared phantom gaps such as the navigation manager's search branch. Pages a test opens with `context.newPage()` are recorded too. Fourteen options-page cases were added (legacy import through `migrateConfiguration`, a file that is not JSON, a file that fails the schema, the file picker, every conflict kind of the import dialog in one import, the untouched quality picker, cancel, the password field with its reveal button, the per-channel speed list including a pasted `/channel/` link and a handle resolved by fetching the channel page, the screenshot file-name template with its placeholder validation, the CSS editor's problem list and expand mode, the number input's arrows, keyboard selection in a select), one fullscreen-placement case per declared button (15, from the metadata), deepDarkCSS with all seven custom colours, miniPlayer resize by pointer, every default position and the seek bar's hover preview and click-to-seek, playlistManagementButtons' per-row reset and remove on Watch Later (the reset drops the video from the watch history and the remove-all button follows the count), saveToWatchLaterButton's Innertube fallback when `ytd-app.resolveCommand` is missing and its error tooltip when the request fails, pauseBackgroundPlayers after an in-page navigation, and playerQuality's choice between formats of one level under the fps and premium preferences. That last one runs against stubbed quality data: the player only exposes format ids in `getAvailableQualityData()` to a YouTube Premium account (the user's note; the test profile is not one, and a probe of six videos, 60 fps and premium ones included, showed entries without a `formatId`), so on this profile the selection code never runs against the live site and the existing fps and premium cases skip for want of a candidate. With a Premium profile they would run for real; the stub keeps the selection logic itself under test either way. Three product defects were found on the way and fixed (3.10): the conflict dialog's untouched quality picker stored `"playerQuality"` as the quality, the legacy `enable_remove_all_watched_button` flag was migrated onto the wrong button, and `updateConfigAtPath` logged debug lines. Unreachable code was named rather than covered: the navigation manager's `/c/` and posts and streams branches (the page-type detector never yields those types), the ambient-mode shorts branch (3.10, open) and the slider control (no setting uses it). The user then objected to a hand-written list of button features in the new fullscreen loop, which was wrong in two entries; the list now comes from the metadata, and the same treatment was given to the rest of the test code (3.11): locale strings come from the locale file, and the extension's own ids, classes and selectors from constants modules that product and specs share.
- 2026-09-07, later: the user set a floor of 80 % for branch coverage (it stood at 70.9 %). Of the unmet branches, 591 were guards of the form "if the element is missing, return", which a healthy page never trips one feature at a time, so a new spec, `everyFeature.spec.ts`, switches every feature and every button on together: on watch through an in-page round trip and a full disable, on shorts, on a playlist, on a live stream, on watch after a reload with theater mode and a related-video navigation, with every button below the player, in the feature menu and in fullscreen, across the feed and channel pages by full loads, and on pages stripped of their player, their controls and owner row, their video element and sidebar, the shorts player, or the playlist list and header; each case asserts that the extension raises no error on the page (twelve cases, all passing). Alongside: the options page's every checkbox switched on and off, the read-only per-channel speed list, a colour typed into a deep dark picker, the CSS editor's warning marker, the two remaining date formats and the channel and chapter placeholders of the screenshot name, the hover-to-click switch of the feature menu, a manually opened mini player closed from its button, the artist-channel exclusion of videoHistory, a modified and a secondary click on a timestamp link, a single-format level in the quality stub, odd value shapes in the legacy import, and hideShorts on every channel tab after the page detector learned the posts and streams tabs (3.10). The registry's performance tracker, switched by `DEV_MODE`, is left out of the report like the devtools. A fresh instrumented run of the whole suite (781 cases, 95 min: 776 passed, 1 flaky, 2 failed, 2 skipped; the flaky and one failure were live-stream timing under the profiler, the other failure the videoHistory navigation case, a product race run to ground and fixed afterwards, 3.10) reads 74.8 % of branches (`E2E-COVERAGE.md`), from 70.9 %; 154 of the remaining unmet branches are dead code (3.10), without which the same records read 77.1 %. The rest are guards and fallbacks a test can only reach by breaking the page further, so 80 % needs a product decision on the dead code, or more page-breaking cases of the same kind.
- 2026-09-07, evening: the user ruled on the dead-code list of 3.10 item by item. Kept on purpose, as ways of doing things that may be needed or for compatibility: the `/c/name` channel URL forms (channels without a handle still use them), `excludePages`, the `buttons` config shape, the slider control, the generator's text-input branch, the extra `modifyElementsClassList` overloads, the `ytd-watch-grid` layout and the embedded script's error reporter; the coverage ceiling stands accordingly. Four items were acted on. The button controller's icon type checks became a type guard (`isToggleIcon`), so no branch is left for an icon that is neither an SVG nor an on/off pair, and `modifyIconForLightTheme` lost its redundant `isToggle` parameter. The metadata validation moved out of the page: a pre-build step (`src/pipeline/steps/validateFeatureMetadata.ts`) loads every `index.metadata.ts` and runs the same checks (`featureMetadataValidation.ts`), the registry runs them at load in development builds only, and the setting-id check turned out never to have failed at all (it parsed a probe object against a non-strict schema, which strips unknown keys, and ids carry the feature id as their first segment), so it now walks the schema's fields with the feature prefix stripped; 59 features pass. Ambient mode on shorts and the audio track feature on shorts were switched on (both now pass `pageTypes` with shorts to the player manager), and the ambient feature's shorts branch was rewritten for today's page: the row is a `yt-list-item-view-model` with a `button[role=switch]` in the reel's "more" sheet, which YouTube fills a few frames after the press and only offers in the dark theme and on the reel a page loads on, so the feature opens the sheet out of sight, waits for its rows, reads the switch, clicks it when needed, and takes a sheet without the row as nothing to do; at page start-up YouTube fills the sheet before it adds the row (seen in a trace of the one flaky reload case), so the start-up run lets the first five such sheets count as not ready yet, and a light-theme page is left alone at once. YouTube's switch turns off on a click but does not turn back on within a page session (trusted click, script click and keyboard alike), so the restore on disable cannot be verified there; three shorts cases pass, the watch cases are unchanged, and the audio track spec's three shorts cases pass once its helpers looked for `#shorts-player`. The deep dark authors' credit renders again: a group's attribution is collected per section and rendered above the settings, since the flattening keeps only the group's children (options case added). Then, on the user's note that the conflict dialog's feature labels were hard-coded English, every user-facing literal outside the locale was hunted down (3.11).
- 2026-09-07, night: issue #1411 (from discussion #1410) reported that Reverse Playlist loses its reversed order, and sometimes the playlist itself, after an automatic transition. The four defects behind it and the fix are in 3.10, verified against YouTube's playlist-manager code and against the fixture, whose video is the last of a 437-video playlist of which YouTube loads a window of 21. `playlistReverseButton.spec.ts` gained five cases: natural autoplay from the playlist's last video into the reversed order with `list=` kept and the new page turned over, two more legs from pages opened near the end of their video, the player's Next and Previous entries and a Next click, YouTube handing the panel forward data again (stood in for from the page), and Back through the history holding the order. Its readers of playlist data take the panel's live data, as the feature does, and the fresh-page legs open their video with `t=` rather than seeking, per the note on seeking. The spec passes 21 of 21 on three workers without retries (2.4 min), the five new cases 5 of 5 on their own. Two earlier runs failed for a reason outside the code: the worktree's `nodemon` watcher (`npm run dev`) was running, so every source file copied in started its own `npm run build` under a manual one, and with two pipelines interleaved one's copy step deleted `dist/temp` before the other's manifest step listed the `src/index*.js` chunks, so the built extension's manifest whitelisted none of them and every feature failed to register ("Denying load of chrome-extension://…/src/index.js"). With the watcher running, the way is to change the source, wait for its build (the manifest lists 65 chunks and `dist/temp` is gone, about 35 s) and only then start a run. A throwaway spec printing the panel, manager and page data before and after a click found the last piece: the feature now reads the panel's own data object, and handing that same object back re-rendered nothing, so consumers are given a copy.
...
- `featurePlayerManager` defaults `pageTypes` to `["watch", "live"]`, so the shorts path of `automaticallyDisableAmbientMode` and `defaultToOriginalAudioTrack` never ran. **Fixed** (`fix(automaticallyDisableAmbientMode)`, `fix(defaultToOriginalAudioTrack)`: both pass `pageTypes` with shorts; the ambient shorts branch was rewritten for the current sheet, see the status entry of 2026-09-07, evening).
- `featureMetadataRegistry` validated setting ids against a non-strict schema, so an id that names no field passed. **Fixed** (`refactor(registry)`: the check walks the schema's fields with the feature prefix stripped, and the whole validation runs as a build step, at load only in development builds).
- `SettingsGenerator` flattened every group into its children before rendering, so the deep dark theme's author credit declared in its metadata never reached the page. **Fixed** (`fix(settings)`: a section renders the attribution of its groups above its settings).
- `ButtonController` kept type checks for an icon that is neither an SVG nor an on/off pair, which the types forbid. **Fixed** (`fix(buttonController)`: `isToggleIcon` type guard).
...
- `playlistReverseButton` lost the reversed traversal in playback: after an automatic transition the reversal was often gone, and the next video could open without its `list=` (issue #1411, from discussion #1410, a user's write-up with a working userscript; analysed against YouTube's own playlist-manager code, `navigateToAutoplayWatchEndpoint_`, `getVideoEndpoint_` and its set builder, and against the fixture, where `dQw4w9WgXcQ` is the last of the 437 videos of `UUuAXFkgsw1L7xaCfnd5JJOw` and YouTube loads a window of 21). Four defects sat behind it. The autoplay sets were rotated the wrong way round: `nextButtonVideo` took the old `autoplayVideo`, so the player's Next and Previous both pointed forward, and on the playlist's last video Next held YouTube's radio suggestion, an endpoint without a playlist, which is how a video could open outside the playlist. The navigation hook recorded the previous playlist position and waited for it to change, but the watch page's type detection waits for the player to switch first, by which time YouTube's data for the new video had usually arrived, so the wait timed out and the setup returned without reversing or injecting the button; the same check collided at the midpoint of even-length playlists. The re-apply was an unconditional toggle, so a Back navigation, which serves YouTube's cached and already reversed page, turned it back. And nothing ran after a navigation's one shot, while YouTube hands the panel, the playlist manager and the player its own copy of the playlist again (its response to a navigation, more of a long playlist, a queue or miniplayer change); that copy never reaches the page data the feature read, so the feature's own check could not see the reversion. **Fixed** (`fix(playlistReverseButton)`: the NORMAL and LOOP sets are derived from the panel's neighbours in the new order, the way YouTube builds them itself, entries outside the playlist dropped and the loop set wrapping only when the whole playlist is loaded; the data is trusted when the item YouTube marks as playing is the video in the address; the reversal is applied only when the live order is the wrong way round; the panel and the manager are read first, and `yt-playlist-data-updated` and `yt-page-data-updated` are answered by restoring the order, with a cap; consumers are handed a fresh copy so that the panel re-renders; the reversed position is kept relative to the loaded window, which is what the manager compares with the window's size to allow a next video; a setup generation makes work queued by an earlier setup stand down after a cleanup, and a disable cleans up before it un-reverses, since the un-reversal itself is a hand-over the listeners must not answer). The spec gained five cases: natural autoplay from the playlist's last video, two more legs from pages opened near their end, the player's Next and Previous entries and a Next click, YouTube handing forward data back, and Back through the history; its readers take the panel's live data, as the feature does.

- The import conflict dialog seeded every conflict's selection with the first feature's id, including the "auto quality" conflict, whose picker offers quality levels. Applying the dialog without touching that picker stored the string `"playerQuality"` as the quality. **Fixed** (`fix(options)`: the picker starts on its first option and that is what an untouched apply stores).
- `migrateConfiguration` mapped the legacy `enable_remove_all_watched_button` flag onto the per-item remove button, because it tested `key.includes("remove")` before anything else and both legacy keys contain "remove"; the remove-all button was never migrated and the remove button took the wrong flag when both were present. **Fixed** (`fix(config)`: the remove-all key is matched first).
- `updateConfigAtPath` (`src/utils/config/utils.ts`) logged `[DEBUG-BP]` lines to the console on every setting change made in the options page. **Fixed** (`fix(config)`: the debug logging is gone).
- `videoHistory` lost the video after an in-page navigation in three ways, which the navigation case of its spec showed as a one-in-three failure once the related list led with a Mix: it took the next video id the player reported within five seconds, which is the pre-roll's when one plays and still the previous video's on a slow load; it looked the video element up once, and YouTube rebuilds the player when a Mix is left for a plain watch page, so the element was not there yet; and its artist check read the owner row of the page being left, whose badge made the next video count as an artist's. **Fixed** (`fix(videoHistory)`: the navigation path waits for the page's own video with no ad showing like start-up does, waits for the video element, and reads the badge only once the owner row names the channel the player reports). The navigation case now passes eight of eight, in parallel with the rest of the spec.
- `getCurrentPageType` (`src/utils/url`) never returned `channel_posts` or `channel_streams`, although both are declared page types, `hideShorts` lists them in its scope and the navigation manager builds signatures for them; on a channel's posts and streams tabs no page-gated feature ever ran. **Fixed** (`fix(url)`: `@handle/posts` and `@handle/streams` are detected; the hideShorts spec opens every channel tab).

Still open (observed while writing tests, not fixed on this branch):

- The branch-coverage work listed 154 unmet branches as dead code: the `/c/<name>` channel forms and the `excludePages` check of the navigation manager, the `buttons` config shape in the legacy migration and the button controller's placement lookup, the slider control, the generator's text-input branch, three `modifyElementsClassList` overloads, the button controller's icon type checks and `ytd-watch-grid` branches, the metadata validation, the embedded script's error reporter, the ambient shorts branch and the group rendering with attribution. **Ruled on** by the user on 2026-09-07: the URL forms (channels without a handle still use them), `excludePages`, the `buttons` shape, the slider, the text-input branch and the overloads stay as intended ways of doing things, the grid layout for compatibility, and the error reporter is not to be touched; the icon checks, the validation, the shorts branch and the attribution were fixed or improved (status entry of that evening). The kept items are a ceiling on branch coverage, not a cleanup list.
- `playbackSpeedButtons` clamps at a maximum of 16 and shows "Can't increase further" there, but YouTube's player API, which the buttons read and write the rate through, caps at 2, so neither the clamp nor the title can be reached; the feature's ceiling should follow the player's.
...
### 3.11 Test code that copied product values

Found on 2026-09-07 when a hand-written list of button features in `buttonController.spec.ts` turned out to be wrong in two of eight entries (the volume boost button is named `volumeBoostButton`, and its switch is the feature's, not the button's). A scan of every spec and helper for string literals that also occur in product code or in the locale file found the same pattern in three forms; the scan script is not kept, its findings are.

- **Lists of features written by hand.** The fullscreen placement loop now takes its buttons from the feature metadata: `src/utils/_tests/metadata.ts` loads every `index.metadata.ts` the extension registers (top-level `await` in the spec) and `listFeatureButtons` derives the element id, the switch and the placement keys per button, so the loop found 15 buttons across 11 features where the list had 8, and a feature added or renamed changes the loop with it. The one thing metadata does not carry, a button's `shouldRender` condition, is a map keyed on `FeatureKeys` so a rename is a type error. The Options spec's section-title check asked a regular expression over the metadata source files; it now asks the metadata objects themselves, with the locale file as the translator (`localeSelector`).
- **Locale strings typed out.** 34 sites in 14 specs held English strings ("Loop on", "Blocked keyword", "Settings imported successfully") that the UI renders from `en-US.json`. They now come from `localeText(path)` in `src/utils/_tests/locale.ts`, whose path parameter is typed against the shipped locale typings, so a reworded or moved string fails the typecheck instead of a run.
- **Extension-owned identifiers copied.** Ids, classes, attributes and selectors the extension itself creates were repeated in the specs and in three helpers, some with a "mirrors index.ts" comment. Each now has one exported home that product and spec import alike: `miniPlayer/constants.ts` (overlay, handle, sentinel, active class, seek bar class), `playlistManagementButtons/constants.ts` (item and header selectors, the three button names), `keywordBlocklist/constants.ts` (blocked attribute, hover class), `playlistReverseButton/constants.ts` (button and container ids), `automaticallyDisableAmbientMode/constants.ts` (the menu entry selectors, previously duplicated in `_tests/ambient.ts`), `buttonController/constants.ts` (the player control containers, previously duplicated in `_tests/constants.ts`), `saveToWatchLaterButton/constants.ts` (the icon names, previously duplicated in `_tests/watchLater.ts`), `utils/video` (the watch-progress selectors), `utils/deep-dark-theme/dom.ts` (the data attributes), and at first two label modules for the conflict dialog and the number arrows, which were user-facing English outside the locale and went there instead (next bullet). `_tests/ambient.ts` re-exports the product's `settingsPanelMenuSelector` instead of its own copy.
- **User-facing strings outside the locale.** The user pointed out that the conflict dialog's feature labels were hard-coded English, and a scan of the components, pages, features and utilities for literals shown to users found the rest: the dialog's six feature names and its modifier key names, the number input's arrow labels, the select's "Select an option" placeholder, the settings header's icon text, the modifier key names built in the two scroll wheel features' metadata, and a fallback title in keywordBlocklist. The feature names now come from the feature metadata, the label of each feature's enable switch (`metadataRegistry.getSettingLabel`), so the dialog names a feature the way the settings page does; the others got keys under `pages.options.extras` (`modifierKeys`, `numberInput`, `select`, `iconAlt`), added to all 23 locale files in English the way earlier keys were; the fallback went, since i18next never returns an empty string. The devtools panel (developer-only) and the README generator keep their English. The specs read the same sources: `featureSettingLabel` in `_tests/metadata.ts` for the dialog and `localeText` for the rest, with the dialog's radios located by role since their names now match the page's checkboxes.
- **Left as they are, on purpose.** Configuration paths (`"playerQuality.quality"`) are typed `Path<configuration>` and feature button ids are typed `FeatureButtonId`, so a rename already fails the typecheck. `pageTypeRecord` and `placementRecord` `satisfy` the product's own types. `PlayerStates` mirrors an enum of a package that is a type-only dependency here (documented in place). YouTube's own selectors (`ytd-watch-flexy`, `.ytp-settings-button`, `.html5-main-video`) describe the page, not the extension: product and test break together when YouTube changes them whichever way they are written, and a test that finds the element its own way can still tell a wrong selector in the product from a changed page. Not yet moved, same pattern to apply: the class names of `hideEndScreenCards`, `hideScrollBar`, `automaticallyShowMoreVideosOnEndScreen`, `maximizePlayerButton` (`yte-maximized`, the size-button state, the CSS variables), the chat style id of `hideArtificialIntelligence`, the ids of the remaining-time span, the playlist-length panel, the feature menu and the tooltips, and the text-target selectors of `keywordBlocklist`.
