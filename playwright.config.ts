...
// TODO: migrate spec config keys from flat `enable_*` ids to dev's nested `<feature>.enabled` schema
...
import type { ButtonPlacement, FeatureMenuItemId, ModifierKey } from "./src/types/index";

type FeatureButtonId = FeatureMenuItemId | (string & {});
checkTests();
...
export async function adjustWithScrollWheel({
	...
	const { scrollWheelVolumeControl, scrollWheelSpeedControl } = getDefaultConfiguration();
	const defaultSteps = (controlType === "volume" ? scrollWheelVolumeControl : scrollWheelSpeedControl).steps;
	...
	expect(scrollWheelAdjustment).toBe(defaultSteps.toString());
	...
	const expectedValue = value + (direction === "up" ? 1 : -1) * defaultSteps;
...
export async function disableFeature(page: Page, feature: string) {
...
export async function enableFeature(page: Page, feature: string) {
	// Wait for the checkbox to appear
	const checkbox = waitForSelector(page, feature);
...
export async function getClosestQuality(page: Page, quality: YoutubePlayerQualityLevel) {
	...
	const closestQuality = chooseClosestQuality(quality, availableQualityLevels, "lower");
...
export async function getSelected<P extends Page, K extends string>(page: P, id: K) {
...
export async function selectOption<P extends Page, K extends string, V extends number | string>(page: P, id: K, value: V) {
...
export async function setValue<P extends Page, K extends string, V extends number>(page: P, key: K, value: V) {
...
export function waitForSelector(page: Page, selector: string) {
