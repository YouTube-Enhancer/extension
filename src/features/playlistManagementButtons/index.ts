...
async function setupPlaylistManagementButtons(config: configuration["playlistManagementButtons"]) {
	const {
...
export default createFeature({
	...
	onEnable: async (config) => {
		await setupPlaylistManagementButtons(config);
	},
	onNavigate: async () => {
		cleanupPlaylistManagementButtons();
		await setupPlaylistManagementButtons(registry.configManager.getLast("playlistManagementButtons"));
	}
});
