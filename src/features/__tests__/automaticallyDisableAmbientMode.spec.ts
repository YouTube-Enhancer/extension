...
/**
 * Drives ambient mode through the player settings menu, the same control the feature toggles. Tests use it to
 * establish that ambient mode really was on, so that a later `toBe(false)` can only pass because the feature acted.
 */
async function setAmbientState(page: Page, desired: boolean): Promise<void> {
	await expect
		.poll(
			...
			{ intervals: [500], message: `expected ambient mode to be turned ${desired ? "on" : "off"}`, timeout: 20000 }
		)
		.toBe(true);
}
